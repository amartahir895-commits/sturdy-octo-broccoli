...
let handler = async (m, {
  ...
  if (handler.limit && user.limit <= 0 && !isOwner)
    return m.reply(global.mess?.limit || '❌ Your limit is over')

  if (!text)
    return m.reply(
      `❌ Example:\n${usedPrefix + command} https://whatsapp.com/channel/xxxxx 😂,🔥,❤️`
    )
  ...
  try {
    m.reply('⚡ Sending reactions, please wait...')
    ...
    if (data?.message) {
      ...
      m.reply('❌ Failed to send reactions')
    }

  } catch (err) {
    console.error('RCH ERROR:', err?.response?.data || err.message)
    m.reply('❌ Error while sending reactions')
  }
}
